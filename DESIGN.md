...
Warm editorial. A private companion that happens to be rigorous: bone and ink surfaces lit by a soft phase aura, a characterful display serif, hairline section rules, and mono data marks — with one signature piece of light, the cycle ribbon, that makes the body's rhythm visible at a glance.
...
- **Breakpoints**: `< 360` is compact (tighter gutters, smaller tab and flow-tile labels); `< 380` shrinks calendar tiles; `>= 860` is the only two-column layout (Today's hero beside the ribbon).
- **Never truncate a label to preserve a layout.** The ribbon uses a wrapping legend rather than labels pinned under each segment, because a narrow phone cannot give "Winding down" the width its segment implies.
...
- **Data draws itself in** — the ribbon marker, range rails, progress fills, and cycle bars animate from zero on mount (`useDrawIn`).
